...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (globalForm && globalInput) {
    globalForm.addEventListener("submit", (e) => {
      ...
      // If you're in a subfolder, this still works if products.html is in same folder.
      window.location.href = `products.html?q=${encodeURIComponent(q)}`;
    });
  }

  // Only run products filtering on products.html (prevents errors on other pages)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  input.addEventListener("input", () => {
    ...
    typingTimer = setTimeout(() => {
      ...
      // If already on products page → filter instantly
      else if (typeof applySearchFilter === "function") {
        applySearchFilter(q);
      }
    }, delay);
  });

  // Auto-apply search if user came from another page with ?q=
  const params = new URLSearchParams(window.location.search);
  const query = params.get("q");

  if (query && typeof applySearchFilter === "function") {
    input.value = query;
